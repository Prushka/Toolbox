...
func canAcceptRenamedPort(target emulation.Port) bool {
...
func waitForPort(ctx context.Context, target emulation.Port, allowRenamedPort bool) (emulation.Port, error) {
	...
	for {
		...
		select {
		case <-ctx.Done():
			return emulation.Port{}, fmt.Errorf("wait for USB re-enumeration: %w", ctx.Err())
		case <-ticker.C:
		}
	}
}

func findTarget(target emulation.Port, ports []emulation.Port) (emulation.Port, bool) {
